...
- **Config keys are env-var NAMES, not secrets.** An ALL_CAPS `apiKey` value is resolved from `process.env` at runtime (`src/utils/resolve-env.ts`); anything else is used literally. Keep `packages/shared/src/env/registry.ts` current and rerun `bun run env:examples` when you add a variable.
